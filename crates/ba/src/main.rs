...
fn main() -> anyhow::Result<()> {
    ...
    unsafe { GetKeyboardLayoutNameA(&mut pwszklid).context("Failed to get keyboard layout")?; }
    let keyboard_layout = CStr::from_bytes_with_nul(&pwszklid)?.to_str()?;
    if keyboard_layout != "0000100C" {
        tracing::error!("Expected '0000100C' for keyboard layout, got '{}'", keyboard_layout);
        Err(anyhow::anyhow!("Invalid keyboard layout, switch to Swiss-French"))?
    }
